...
const Weather = () => { // Component name should start with an uppercase letter

  const [city, setCity] = useState('');
  const [weather, setweather] = useState();
  const [error, seterror] = useState('');

  const API_KEY = "a86b535cda7b7087f58b922f645f0f98";
  ...
  async function fetchData(){
    try{
      ...
      if(response.ok){
        ...
        seterror('No Data found, Kindly Enter Valid City Name.');
      }
    }catch(error){
